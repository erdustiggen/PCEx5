...
struct workItemGPU {
		float4 scale_distanceOffset;
    workItemGPU(float& scale_, float3& distanceOffset_) {
				scale_distanceOffset.w = scale_;
				scale_distanceOffset.x = distanceOffset_.x;
				scale_distanceOffset.y = distanceOffset_.y;
				scale_distanceOffset.z = distanceOffset_.z;
		}
    workItemGPU() {
			scale_distanceOffset.w = 1;
			scale_distanceOffset.x = 0;
			scale_distanceOffset.y = 0;
			scale_distanceOffset.z = 0;
				  	  ...
				  	  float const rotationAngle = 0)
{
	...
	positionOffset.w *= 3.0f;
	mat4x4 scaleMatrix(
		positionOffset.w/*X*/,	0,			0,				0,
		0, 			positionOffset.w/*Y*/, 0,				0,
		0, 			0,			positionOffset.w/*Z*/, 	0,
		0, 			0,			0,				1);
...
__global__ void renderMeshes(
	...
	runVertexShader(v0, objectToRender.scale_distanceOffset, width, height);
	runVertexShader(v1, objectToRender.scale_distanceOffset, width, height);
	runVertexShader(v2, objectToRender.scale_distanceOffset, width, height);
